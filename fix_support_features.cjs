...
// 1. Rewrite Support.tsx
const userSupport = `import React, { useState, useEffect } from 'react';
...
// 2. Rewrite AdminSupport.tsx
const adminSupport = `import React, { useState, useEffect } from 'react';
...
export default function AdminSupport() {
  ...
  const deleteChat = async () => {
    ...
    await supabase.from('support_tickets').update({ status: 'deleted' }).eq('id', activeUser.ticket_id);
    setActiveUser(null);
    fetchThreads();
  };
...
console.log("Rewrote both support files.");
