...
import { Search, ArrowUpRight, ArrowDownRight, Star, Plus } from 'lucide-react';
    ...
    <div className="animate-fade-in">
      ...
      <header className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-brand-text-gray">Live market data for {COINS.length}+ supported assets.</p>
        </div>
      </header>
              ...
              {filteredCoins.map((coin) => (
                <tr key={coin.id} className="hover:bg-gray-50/50 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{coin.symbol} • {coin.type}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-right">
                    <p className="font-bold text-brand-dark">${coin.price.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 6})}</p>
                  </td>
                  <td className="px-6 py-4 text-right">
                  ...
                  <td className="px-6 py-4 text-right hidden md:table-cell">
                    <p className="text-sm font-medium text-gray-600">${(coin.marketCap / 1e9).toFixed(2)}B</p>
                  </td>
                  <td className="px-6 py-4 text-right">
                    <button 
